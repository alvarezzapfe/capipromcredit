...
function ModalAlta({ onClose, onSaved, isMobile }: { onClose: () => void; onSaved: () => void; isMobile?: boolean }) {
  ...
  // Capital base: sum of disposiciones or monto
  const capitalBase = form.tipo_disposicion === "multiple" && disposiciones.length > 0
    ? disposiciones.reduce((s, d) => s + (Number(d.monto) || 0), 0)
    : Number(form.monto) || 0;

  // Preview
  const puedePrevisualizar = capitalBase > 0 && tasaEfectiva > 0 && Number(form.plazo_meses) > 0;
  ...
  async function guardar() {
    ...
    const { data: credito, error: errCred } = await supabase.from("creditos").insert({
      ...
      monto: Number(form.monto), tasa_anual: tasaFinal,
      ...
      tipo_disposicion: form.tipo_disposicion,
    }).select().single();

    if (errCred || !credito) { setGuardando(false); setError("Error: " + (errCred?.message ?? "")); return; }

    // Disposiciones
    if (form.tipo_disposicion === "multiple" && disposiciones.length > 0) {
    ...
    await supabase.from("bitacora").insert({ entidad: "credito", entidad_id: credito.id, accion: "creado", detalle: { folio, monto: Number(form.monto), cupones: tabla.length }, usuario: operador });
    setGuardando(false);
    onSaved();
  }
    ...
    <Overlay onClose={onClose}>
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 0 }}>
        <div style={{ padding: isMobile ? "24px 20px" : "30px 30px", borderRight: isMobile ? "none" : "1px solid var(--line)" }}>
          <h2 style={{ fontSize: 20, fontWeight: 600, color: "#0a1628", marginBottom: 16 }}>Originar crédito</h2>
          <div style={{ display: "grid", gap: 12 }}>

            {/* A. Acreditado */}
            {sectionTitle("Acreditado")}
            <div className="field"><label>Nombre / razón social *</label><input className="input" value={form.acreditado} onChange={(e) => set("acreditado", e.target.value)} /></div>
        ...
        <div style={{ padding: isMobile ? "20px" : "30px 26px", background: "#f4f6f8", maxHeight: "82vh", overflowY: "auto" }}>
          ...
          : preview.length === 0 ? <p style={{ color: "var(--text-faint)", fontSize: 13.5 }}>Captura monto, tasa y plazo para ver la tabla.</p>
          : (
            <>
              <p style={{ color: "var(--text-dim)", fontSize: 13, marginBottom: 14 }}>
                {preview.length} cupones · {graciaNum > 0 ? `${graciaNum}m gracia · ` : ""}primer pago <span className="mono" style={{ color: "var(--amber)" }}>{mxn(preview[0].pago_total)}</span>
              </p>
              <div style={{ overflowX: "auto" }}>
